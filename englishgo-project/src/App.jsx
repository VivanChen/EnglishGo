...
import { archiveStudyDay, dailyProgress, learningStreak, localDay } from "./data/studyProgress.js";
import { buildDailyMiniDeck } from "./data/learningJourney.js";
...
const ReviewGarden=lazy(()=>import("./features/ReviewGarden.jsx"));
const GrammarWorkshop=lazy(()=>import("./features/GrammarWorkshop.jsx"));
const SpeakingStudio=lazy(()=>import("./features/SpeakingStudio.jsx"));
const SongsStudio=lazy(()=>import("./features/SongsStudio.jsx"));
...
// ═══ ACHIEVEMENTS ═══════════════════════════════════════════════════
const ACH_DEFS = [
  {id:"first_card",name:"初心者",desc:"完成第一輪單字卡練習",icon:"🌟",metric:"srsRounds",target:1,unit:"輪",module:"srs",group:"learn",action:"練一輪 5 張單字卡",check:s=>s.srsRounds>=1},
  {id:"streak3",name:"三日不懈",desc:"連續學習 3 天",icon:"🔥",metric:"streak",target:3,unit:"天",module:"srs",group:"learn",action:"留下今天的小腳印",check:s=>s.streak>=3},
  {id:"streak7",name:"一週達人",desc:"連續學習 7 天",icon:"💎",metric:"streak",target:7,unit:"天",module:"srs",group:"learn",action:"留下今天的小腳印",check:s=>s.streak>=7},
  {id:"xp100",name:"經驗滿載",desc:"累積 100 XP",icon:"⭐",metric:"xp",target:100,unit:"XP",module:"srs",group:"learn",action:"做一份單字小任務",check:s=>s.xp>=100},
  {id:"xp500",name:"學霸之路",desc:"累積 500 XP",icon:"🏆",metric:"xp",target:500,unit:"XP",module:"srs",group:"learn",action:"做一份單字小任務",check:s=>s.xp>=500},
  {id:"quiz_perfect",name:"零失誤",desc:"測驗拿滿分",icon:"💯",metric:"perfectQuiz",target:1,unit:"次",module:"quiz",group:"learn",action:"試試單字小測驗",check:s=>s.perfectQuiz>=1},
  {id:"dict5",name:"聽力新手",desc:"完成 5 次聽寫",icon:"🎧",metric:"dictDone",target:5,unit:"次",module:"dictation",group:"read",action:"聽一段短句",check:s=>s.dictDone>=5},
  {id:"scram5",name:"語序大師",desc:"完成 5 次句子重組",icon:"🧩",metric:"scramDone",target:5,unit:"次",module:"scramble",group:"game",action:"組一個英文句子",check:s=>s.scramDone>=5},
];
...
function makeUtterance(t,l="en-US",r=0.9,opts={},token=_speechToken){
  ...
  if(opts.playbackRate)u.__englishGoPlaybackRate=opts.playbackRate;
...
const S={btn:{padding:"12px 24px",borderRadius:12,border:"none",fontWeight:700,fontSize:16,cursor:"pointer",fontFamily:"inherit",transition:"transform .16s ease, box-shadow .16s ease, background .16s ease"},
  card:{background:"var(--color-background-primary,#fff)",borderRadius:18,border:"1px solid var(--color-border-tertiary,#e0dfd9)",boxShadow:"var(--eg-card-shadow,0 12px 32px rgba(20,66,52,.08))"},
...
function ModuleLoading({label="載入功能中..."}){
...
function triggerRewardBurst(opts){
  if(document.documentElement.dataset.egCalm==="true"||window.matchMedia?.("(prefers-reduced-motion: reduce)").matches)return;
...
export default function App(){
  ...
  const[petStartTab,setPetStartTab]=useState("home");
  ...
  const[calm,setCalm]=useLS("calm",false);
  const[quiet,setQuiet]=useLS("quiet",false);
  const[lastLevel,setLastLevel]=useLS("lastLevel",null);
  const[lastActivity,setLastActivity]=useLS("lastActivity",null);
  const comfort={dark,setDark,calm,setCalm,quiet,setQuiet};
  useEffect(()=>{document.documentElement.dataset.egCalm=String(calm);document.documentElement.dataset.egQuiet=String(quiet);if(_battleAudioCtx&&_battleAudioCtx.state!=="closed"){if(quiet)_battleAudioCtx.suspend?.();else _battleAudioCtx.resume?.()}},[calm,quiet]);
  useEffect(()=>{if(lv)setLastLevel(lv)},[lv]);
  ...
  // Roll each local day once, including a page left open overnight.
  const dailyRef=useRef(daily);dailyRef.current=daily;
  const rollStudyDay=useCallback(()=>{
    const now=new Date(),previous=dailyRef.current;
    if(localDay(previous?.date)===localDay(now))return;
    const yesterday=new Date(now);yesterday.setDate(yesterday.getDate()-1);
    const next={target:dailyProgress(previous).target,done:0,date:now.toDateString()};
    dailyRef.current=next;
    setHistory(current=>archiveStudyDay(current,previous));
    setStreak(current=>localDay(previous?.date)===localDay(yesterday)&&dailyProgress(previous).done>0?Math.max(1,Number(current)||1)+1:1);
    setDaily(next);
  },[setHistory,setStreak,setDaily]);
  useEffect(()=>{
    rollStudyDay();const timer=setInterval(rollStudyDay,60000);
    const visible=()=>{if(!document.hidden)rollStudyDay()};
    window.addEventListener("focus",rollStudyDay);document.addEventListener("visibilitychange",visible);
    return()=>{clearInterval(timer);window.removeEventListener("focus",rollStudyDay);document.removeEventListener("visibilitychange",visible)};
  },[rollStudyDay]);

  // Returning alone does not complete a learning-day achievement. Keep earned badges forever.
  useEffect(()=>{
    const values={...stats,xp,streak:learningStreak(streak,daily)};
    const earned=ACH_DEFS.filter(a=>!achUnlocked.includes(a.id)&&a.check(values));
    if(earned.length){setAchUnlocked(current=>[...new Set([...current,...earned.map(a=>a.id)])]);setShowAch(earned.at(-1))}
  },[xp,streak,stats,daily.done,daily.date,achUnlocked]);
  ...
  useEffect(()=>{
    ...
    if(loginBonus.lastDate===today){if(!loginBonus.claimed)setLoginBonusModal({streak:loginBonus.streak||1});return;}
    ...
    setLoginBonusModal({streak:newStreak});
  },[]);
  ...
  const claimLoginBonus=()=>{
    if(!loginBonusModal||giftClaimedRef.current)return;
  ...
  useEffect(()=>{
    ...
    document.documentElement.dataset.egTheme=dark?"dark":"light";
    r.setProperty("--color-background-primary",dark?"#223129":"#ffffff");
    r.setProperty("--color-background-secondary",dark?"#2b3c30":"#eff3e9");
    r.setProperty("--color-background-tertiary",dark?"#192720":"#fafaf6");
    r.setProperty("--color-text-primary",dark?"#edf2e8":"#283e33");
    r.setProperty("--color-text-secondary",dark?"#b9c6b4":"#5c6b60");
    r.setProperty("--color-text-tertiary",dark?"#aebda9":"#677660");
    r.setProperty("--color-border-tertiary",dark?"#3b4c40":"#e2e7de");
    r.setProperty("--eg-card-shadow","0 4px 16px rgba(37,74,55,.04)");
  },[dark]);

  useEffect(()=>{
    if(typeof document==="undefined")return undefined;
    document.body.dataset.egModule=lv?(mod||"menu"):"landing";
    const resetScroll=()=>{
    ...
    resetScroll();
    const frame=typeof requestAnimationFrame==="function"?requestAnimationFrame(resetScroll):null;
  ...
  const startMiniMission=()=>{
    const cards=buildDailyMiniDeck(V[lv]||[],levelWeakWords,dateKey());
    navigateEnglishGo({lv,mod:"srs",menuGroup:"learn",sharedWord:null,customDeck:{cards,source:"每日小任務"}});
  };
  ...
  return(
    <div className="eg-app-shell" style={{minHeight:"100vh",background:"var(--eg-app-background,var(--color-background-tertiary,#f8f7f4))",color:S.t1,fontFamily:"'Noto Sans TC','Segoe UI',sans-serif"}}>
      ...
      {!mod&&showAch&&<div className="eg-achievement-toast" role="status"><span aria-hidden="true">{showAch.icon}</span><div><b>新成就 · {showAch.name}</b><small>你的努力，已經收藏在成就牆。</small></div><button type="button" onClick={()=>setShowAch(null)} aria-label="關閉成就提醒">×</button></div>}
        {!mod?<MenuV2 lv={lv} onSelect={openModule} activeGroup={menuGroup} onGroupChange={changeMenuGroup} daily={daily} c={c} xp={xp} coins={coins} streak={learningStreak(streak,daily)} achUnlocked={achUnlocked} weakWords={levelWeakWords} pets={pets} eggs={eggs} onQuickStart={startMiniMission} lastActivity={lastActivity} loginGift={loginBonusModal} claimGift={claimLoginBonus}/>:
         ...
         mod==="colorIsland"?<ColorIslandM lv={lv} onBack={back} onXp={addXp}/>:mod==="wordDash"?<WordDashM lv={lv} onBack={back} onXp={addXp}/>:mod==="whack"?<WhackM lv={lv} onBack={back} onXp={addXp}/>:
         ...
         mod==="weak"?<WeakPage onBack={back} weakWords={levelWeakWords} allWeakWords={weakWords} setWeakWords={setWeakWords} c={c} lv={lv} onCards={cards=>navigateEnglishGo({lv,mod:"srs",menuGroup:"learn",sharedWord:null,customDeck:{cards,source:"單字再練一次"}})} onSearch={()=>openModule("wordsearch","learn")}/>:
         mod==="dashboard"?<Dashboard onBack={back} onOpen={openModule} onQuickStart={startMiniMission} c={c} xp={xp} streak={learningStreak(streak,daily)} stats={stats} daily={daily} weakWords={levelWeakWords} history={history} achUnlocked={achUnlocked} lv={lv}/>:
...
function SettingsPage({onBack,c,gemKey,setGemKey,gifKey,setGifKey}){
  ...
  return(<div>
    ...
    <section style={{...S.card,padding:20,marginBottom:12}}><h3 style={{margin:"0 0 8px",fontSize:17}}>朗讀聲音</h3><p style={{fontSize:13,color:S.t2,lineHeight:1.8}}>點右下角「發音設定」調整聲音與語速。使用瀏覽器備援發音時，可在下方選擇可用的英文聲音。右上方可關閉遊戲音效或減少動畫。</p><VoicePicker/></section>
    ...
    <section style={{...S.card,padding:"18px 20px",marginBottom:12}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"minmax(0,1fr) auto",gap:8,alignItems:"center"}}>
        <input aria-label="Gemini API Key" autoComplete="off" spellCheck={false} inputMode="text" value={gemInp} onChange={e=>setGemInp(e.target.value)} type={showGem?"text":"password"} placeholder="貼上 Gemini API Key" style={inputStyle}/>
    ...
    <section style={{...S.card,padding:"18px 20px",marginBottom:12}}>
      <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:12,marginBottom:12}}>
        <div>
          ...
          <div style={{fontSize:12,color:S.t3,marginTop:3}}>可選，為具體且有可靠圖片對應的 SRS 單字顯示相關 GIF</div>
        </div>
        {status(!!gifKey?.trim())}
      </div>
      <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:12}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"minmax(0,1fr) auto",gap:8,alignItems:"center"}}>
        <input aria-label="Giphy API Key" autoComplete="off" spellCheck={false} inputMode="text" value={gifInp} onChange={e=>setGifInp(e.target.value)} type={showGif?"text":"password"} placeholder="貼上 Giphy API Key，可留空關閉" style={inputStyle}/>
...
function MenuV2({lv,onSelect,activeGroup="learn",onGroupChange,daily,c,xp,coins,streak,achUnlocked,weakWords,pets,eggs,onQuickStart,lastActivity,loginGift,claimGift}){
  ...
  const gradeTheme={groups:{learn:c.cl,read:"#5b819c",game:"#af8847",pet:"#b07d8d",tools:"#8583aa"}};
  useEffect(()=>{
    let active=true;
  ...
  const modules=[
    ...
    {id:"colorIsland",group:"game",icon:"C",t:"顏色生存島",d:"聽英文顏色指令，跑到安全平台",tag:"3D 生存挑戰"},
  ...
  return <LearningDashboard c={c} lv={lv} modules={modules} groups={groups} activeGroup={activeGroup} onGroupChange={setActiveGroup} onSelect={onSelect} todayWord={todayWord} onSpeak={()=>speak(todayWord.w)} daily={daily} xp={xp} coins={coins} streak={streak} pets={pets} eggs={eggs} weakWords={weakWords} onQuickStart={onQuickStart} lastActivity={lastActivity} loginGift={loginGift} claimGift={claimGift} showRewards={showRewards} setShowRewards={setShowRewards} rewardContent={showRewards&&(
...
// ═══ ILLUSTRATED ARCADE ════════════════════════════════════════
const LazyColorIsland=lazy(()=>import("./features/ColorIsland.jsx"));
function ColorIslandM(props){return <Suspense fallback={<ModuleLoading label="正在準備彩色浮島…"/>}><LazyColorIsland key={props.lv} {...props} deps={{speak:speakWebSpeech,stopSpeech,playSound}}/></Suspense>}
const LazyWordDash=lazy(()=>import("./features/WordDash.jsx"));
function WordDashM(props){return <Suspense fallback={<ModuleLoading label="正在搭建 3D 賽道…"/>}><LazyWordDash key={props.lv} {...props} deps={{V,speak:speakWebSpeech,stopSpeech,playSound,loadExtraWords,fetchCloudVocab}}/></Suspense>}
...
function ArcadeM({game,...props}){
  ...
  return <Suspense fallback={<ModuleLoading label="正在準備遊戲島…"/>}><LazyArcadeGames key={`${props.lv}-${game}`} game={game} {...props} deps={deps}/></Suspense>;
}
function WhackM(props){return <ArcadeM game="whack" {...props}/>}
function MatchM(props){return <ArcadeM game="match" {...props}/>}
function BombM(props){return <ArcadeM game="bomb" {...props}/>}

// ═══ QUIZ ═══════════════════════════════════════════════════════════
function QuizM(props){return <Suspense fallback={<ModuleLoading label="準備小任務..."/>}><LearningPractice key={`quiz-${props.lv}`} kind="quiz" {...props} deps={{words:V[props.lv],fetchCloudVocab,speak,stopSpeech,playSound,Hdr,c:LV[props.lv]}}/></Suspense>}
function DictM(props){return <Suspense fallback={<ModuleLoading label="準備聽力練習..."/>}><LearningPractice key={`listening-${props.lv}`} kind="listening" {...props} deps={{sentences:DICT[props.lv],speak,stopSpeech,playSound,Hdr,c:LV[props.lv]}}/></Suspense>}
// ═══ SENTENCE SCRAMBLE (句子重組 v2) ═══════════════════════════════
function ScramM(props){return <ArcadeM game="scramble" {...props}/>}
...
function GrammarM(props){return <Suspense fallback={<ModuleLoading label="準備句型積木..."/>}><GrammarWorkshop key={props.lv} {...props} deps={{rules:G[props.lv],grammarGuide,grammarDrills,generateGrammarAiExplanation,speak,stopSpeech,playSound,Hdr,c:LV[props.lv]}}/></Suspense>}
// ═══ READING ════════════════════════════════════════════════════════
...
function ReadingM({lv,onBack,onXp}){
  const deps={articles:R[lv],translations:R_ZH,useLS,splitReadingSentences,readingKeywords,readingWords,readingEvidence,speak,speakStory,preloadTts,playSound,Hdr,c:LV[lv]};
  return <ReadingPractice key={lv} lv={lv} onBack={onBack} onXp={onXp} deps={deps}/>;
}
// ═══ NOVELS (英文小說閱讀) ═════════════════════════════════════════════
const NOVEL_COUNT=8;
...
async function fetchWeakWordDetails(lv,words){
  const sb=await getSb();if(!sb||!words.length)return [];
  try{const {data}=await sb.from('word_bank').select(WORD_SELECT).eq('level',lv).in('word',words.slice(0,50));return(data||[]).map(mapWord).filter(Boolean)}catch{return []}
}
function WeakPage(props){return <Suspense fallback={<ModuleLoading label="準備複習清單..."/>}><ReviewGarden key={props.lv} {...props} deps={{words:V[props.lv],loadExtraWords,fetchWeakWords:fetchWeakWordDetails,speak,stopSpeech,Hdr,c:LV[props.lv],levelName:LV[props.lv].l}}/></Suspense>}
// ═══ DASHBOARD (學習報告) ═══════════════════════════════════════════
function Dashboard(props){return <Suspense fallback={<ModuleLoading label="整理學習足跡..."/>}><ProgressJournal key={props.lv} {...props} deps={{definitions:ACH_DEFS,Hdr,c:props.c,levelName:LV[props.lv].l,shareLine}}/></Suspense>}
...
function Hdr({t,onBack,cl,extra}){return(<div className="eg-feature-header" style={{display:"flex",alignItems:"center",gap:4,marginBottom:8}}><button type="button" onClick={onBack} style={{background:"none",border:"none",fontSize:12,color:cl,cursor:"pointer",fontWeight:600,fontFamily:"inherit",padding:"6px 8px",minHeight:44,borderRadius:8,WebkitTapHighlightColor:"transparent"}}>← 返回</button><h2 tabIndex={-1} style={{fontSize:16,fontWeight:700,color:S.t1,margin:0,flex:1}}>{t}</h2>{extra}</div>)}
