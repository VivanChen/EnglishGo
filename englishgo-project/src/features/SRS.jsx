...
function VocabularyTopicPicker({lv,levelPools,poolSources,weakWords,onBack,onStart,LV,S,Hdr}){
  const c=LV[lv];
  ...
  const sourceLabel=poolSources?.[lv]==="hybrid"?"完整字庫已就緒":"離線字庫可使用";
  return(<div className="srs-topic-page" style={{"--topic-accent":c.cl,"--topic-accent-2":c.ac,"--topic-soft":c.bg,"--topic-card":S.bg1,"--topic-surface":S.bg2,"--topic-border":S.bd,"--topic-text":S.t1,"--topic-muted":S.t2,"--topic-faint":S.t3}}>
    ...
    <section className="srs-topic-overview" data-testid="srs-topic-overview" aria-labelledby="srs-topic-title">
      <div className="srs-topic-overview-main">
        <div className="srs-topic-overview-icon">🧭</div>
        <div className="srs-topic-overview-copy">
          <h2 id="srs-topic-title">今天想練哪一類？</h2>
          <div className="srs-topic-meta"><strong>{c.ic} {c.l}</strong><span>{levelCards.length} 個單字</span><span>每輪 {VOCABULARY_TOPIC_TARGET} 張</span><span>{sourceLabel}</span></div>
        </div>
      </div>
      {gaps.length===0?<p className="srs-topic-status-note is-good">✅ 所有主題都能開始練習，每一類都有至少 {VOCABULARY_TOPIC_TARGET} 個單字。</p>:<p className="srs-topic-status-note">📋 仍建議補充：{gaps.map(topic=>`${topic.label} ${topic.count} 字${topic.gap?`（缺 ${topic.gap}）`:""}`).join("、")}。</p>}
      {recommended&&<div className="srs-topic-focus"><p>🎯 建議先練：{recommended.label}（{recommended.weakCount} 個待加強），系統會優先排入本輪。</p><button type="button" onClick={()=>onStart(recommended.id)} aria-label={`開始複習推薦主題：${recommended.label}`}>立即練 {recommended.label} →</button></div>}
    </section>
    ...
    <style>{`
      ...
      .srs-topic-overview{padding:15px 16px;margin-bottom:12px;border:1px solid color-mix(in srgb,var(--topic-accent) 24%,var(--topic-border));border-radius:18px;background:linear-gradient(135deg,var(--topic-soft),var(--topic-card));box-shadow:0 12px 30px rgba(20,66,52,.07)}
      .srs-topic-overview-main{display:flex;align-items:center;gap:12px}.srs-topic-overview-icon{width:46px;height:46px;flex:0 0 46px;display:grid;place-items:center;border-radius:15px;background:color-mix(in srgb,var(--topic-accent) 12%,var(--topic-card));font-size:28px;line-height:1}.srs-topic-overview-copy{min-width:0}.srs-topic-overview h2{margin:0 0 7px;font-size:20px;color:var(--topic-text)}
      .srs-topic-meta{display:flex;flex-wrap:wrap;gap:6px}.srs-topic-meta span,.srs-topic-meta strong{padding:3px 8px;border-radius:999px;background:color-mix(in srgb,var(--topic-card) 82%,transparent);color:var(--topic-muted);font-size:11px;line-height:1.45}.srs-topic-meta strong{background:color-mix(in srgb,var(--topic-accent) 13%,var(--topic-card));color:var(--topic-accent);font-weight:900}
      .srs-topic-status-note{margin:10px 0 0;font-size:12px;line-height:1.65;color:color-mix(in srgb,#EF9F27 40%,var(--topic-text))}.srs-topic-status-note.is-good{color:color-mix(in srgb,var(--topic-accent) 30%,var(--topic-text))}
      .srs-topic-focus{display:flex;align-items:center;justify-content:space-between;gap:10px;margin-top:8px;padding:9px 10px;border:1px solid color-mix(in srgb,#EF9F27 44%,var(--topic-border));border-radius:12px;background:color-mix(in srgb,#EF9F27 12%,var(--topic-card))}.srs-topic-focus p{margin:0;color:color-mix(in srgb,#EF9F27 72%,var(--topic-text));font-size:12px;line-height:1.55;font-weight:800}.srs-topic-focus button{flex:0 0 auto;min-height:38px;padding:7px 11px;border:0;border-radius:10px;background:color-mix(in srgb,#8A5A00 78%,var(--topic-accent));color:#fff;font:inherit;font-size:12px;font-weight:900;cursor:pointer}
      ...
      .srs-topic-card:focus-visible,.srs-topic-focus button:focus-visible{outline:3px solid color-mix(in srgb,var(--topic-accent) 48%,transparent);outline-offset:3px}
      ...
      @media(max-width:620px){.srs-topic-overview{padding:13px}.srs-topic-overview-main{align-items:flex-start}.srs-topic-overview-icon{width:40px;height:40px;flex-basis:40px;border-radius:13px;font-size:24px}.srs-topic-overview h2{font-size:17px}.srs-topic-meta{gap:5px}.srs-topic-grid{grid-template-columns:1fr}.srs-topic-card{min-height:104px}.srs-topic-focus{align-items:stretch;flex-direction:column}.srs-topic-focus button{width:100%;min-height:44px}}
    `}</style>
  </div>);
}
...
export default function SRS({lv,onBack,onXp,onDone,trackWeak,gifKey,sharedWord,apiKey,weakWords=[],customCards=null,customSource="",onOpenSettings,deps}){
  ...
  useEffect(()=>{let active=true;(async()=>{setLoading(true);completedRef.current=false;setFlip(false);setFlipAnim(false);setCombo(0);setMaxCombo(0);setSelectedTopic(null);const extra=await loadExtraWords();if(!active)return;let pools=Object.fromEntries(Object.keys(V).map(level=>[level,mergeUniqueWordCards(V[level]||[],extra[level]||[])]));setLevelPools(pools);setPoolSources(Object.fromEntries(Object.keys(V).map(level=>[level,"local"])));if(Array.isArray(customCards)&&customCards.length){const ordered=sortCardsForStudy(customCards,weakWords,sharedWord);setCards(ordered);setDeck(createDeck(ordered));setSelectedTopic("direct");setSrc(customSource||`考試範圍 (${ordered.length}字)`);setLoading(false);return}if(!sharedWord){const cloud=typeof fetchCloudVocabularyCatalog==="function"?await fetchCloudVocabularyCatalog(lv):null;if(!active)return;pools={...pools,[lv]:mergeUniqueWordCards(pools[lv]||[],cloud||[])};setLevelPools(pools);setPoolSources(sources=>({...sources,[lv]:cloud?.length?"hybrid":"local"}));setCards([]);setDeck(createDeck([]));setSrc(`${LV[lv].l}分類字庫`);setLoading(false);return}const cloud=await fetchCloudVocab(lv,20);if(!active)return;if(cloud&&cloud.length>0){
  ...
  useEffect(()=>{if(typeof document==="undefined")return undefined;const reset=()=>{document.documentElement.scrollTop=0;document.body.scrollTop=0;if(typeof window!=="undefined"&&!/jsdom/i.test(window.navigator?.userAgent||""))window.scrollTo({top:0,left:0,behavior:"auto"})};reset();const frame=typeof requestAnimationFrame==="function"?requestAnimationFrame(reset):null;return()=>{if(frame!==null&&typeof cancelAnimationFrame==="function")cancelAnimationFrame(frame)}},[studyActive,lv,loading]);
  ...
  const rate=useCallback(a=>{
    ...
    if(a==="again"||a==="hard")trackWeak(cur.w,1);
    else trackWeak(cur.w,a==="easy"?-2:-1);
    if(a==="easy"||a==="good"){
      onXp();setMascotMood(a==="easy"?"great":"happy");
  ...
  if(done){const{stats,total}=deck;const attempts=stats.again+stats.hard+stats.good+stats.easy;const goodPct=Math.round(((stats.good+stats.easy)/total)*100);return(<div>{showConfetti&&<Confetti/>}<Hdr t="🃏 單字小花園" onBack={leaveStudy} cl={c.cl}/><div style={{textAlign:"center",padding:"32px 16px"}}><div style={{fontSize:56,animation:"bounceIn .5s ease-out"}}>{goodPct>=80?"🏆":goodPct>=60?"🎉":"💪"}</div><h2 style={{fontSize:22,fontWeight:700,color:S.t1,marginTop:8}}>練習完成！共 {total} 張</h2>{selectedTopicMeta&&<div style={{fontSize:13,color:c.cl,fontWeight:800,marginTop:4}}>{selectedTopicMeta.icon} {selectedTopicMeta.label} · {LV[lv].l}</div>}{maxCombo>=3&&<div style={{fontSize:13,color:"#EF9F27",fontWeight:600,marginTop:4}}>🔥 最高 {maxCombo} 連擊！</div>}<div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8,maxWidth:400,margin:"16px auto"}}>{[["🌱 再看看",stats.again,"#927044"],["🤔 有點難",stats.hard,"#A37D40"],["😊 記住了",stats.good,"#437650"],["🌟 很熟悉",stats.easy,"#547E9D"]].map(([l,v,cl])=>(<div key={l} style={{...S.card,padding:"12px 6px",textAlign:"center",borderTop:`3px solid ${cl}`}}><div style={{fontSize:26,fontWeight:700,color:cl}}>{v}</div><div style={{fontSize:11,color:S.t3,marginTop:2}}>{l}</div></div>))}</div><div style={{textAlign:"center",fontSize:13,color:S.t2,margin:"8px 0"}}>掌握率 {goodPct}% · 答題 {attempts} 次</div><div style={{fontSize:14,color:S.t2,marginBottom:14}}>{goodPct>=80?"太厲害了！🌟":goodPct>=60?"表現不錯！繼續加油 💪":"多練習幾次會更好！📖"}</div><button onClick={restartRound} style={{...S.btn,background:c.cl,color:"#fff",marginRight:8,fontSize:14}}>🔄 新一輪</button>{selectedTopicMeta&&<button onClick={leaveStudy} style={{...S.btn,background:S.bg2,color:c.cl,marginRight:8,fontSize:14}}>🧭 換分類</button>}<button onClick={onBack} style={{...S.btn,background:S.bg2,color:S.t1,fontSize:14}}>休息一下，回首頁</button></div></div>)}
  ...
  return(<div className={`srs-page ${dictOpen&&flip?"has-dict":""}`} style={{"--srs-accent":c.cl,"--srs-accent-2":c.ac,"--srs-soft":c.bg,"--srs-card":S.bg1,"--srs-surface":S.bg2,"--srs-border":S.bd,"--srs-text":S.t1,"--srs-muted":S.t2,"--srs-faint":S.t3}}><Hdr t="🃏 單字小花園" onBack={leaveStudy} cl={c.cl} extra={<div style={{display:"flex",gap:4}}><button type="button" onClick={()=>setInfo(!info)} aria-label={info?"關閉操作說明":"開啟操作說明"} title="操作說明" style={{background:"none",border:`1px solid ${S.bd}`,borderRadius:8,padding:"2px 6px",fontSize:12,cursor:"pointer",color:S.t2,minWidth:44,minHeight:44}}>ⓘ</button><label aria-label="匯入 CSV 單字卡" title="匯入 CSV 單字卡" style={{background:"none",border:`1px solid ${S.bd}`,borderRadius:8,padding:"2px 6px",fontSize:12,cursor:"pointer",color:S.t2,minWidth:44,minHeight:44,display:"inline-flex",alignItems:"center",justifyContent:"center"}}>📥<input ref={fr} aria-label="選擇 CSV 單字卡檔案" type="file" accept=".csv" onChange={handleCSV} style={{display:"none"}}/></label></div>}/>
    ...
    {info&&!(gifKey||"").trim()&&<div style={{...S.card,padding:"10px 12px",marginBottom:10,display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,flexWrap:"wrap",fontSize:12,lineHeight:1.5}}>
    ...
    <div className={`srs-study-grid ${dictOpen&&flip?"is-dict-open":""}`}>
      <section data-testid="srs-card" className={`srs-card-shell ${flip?"is-back":"is-front"} ${dictOpen?"is-dict-open":""}`} onClick={handleCardTap}>
        ...
        {!flip?(<>
          ...
          <div className="srs-flow-hint" data-testid="srs-study-guidance"><button type="button" className="srs-reveal" onClick={e=>{e.stopPropagation();handleCardTap()}}>點卡片看答案</button><span>先在心裡想中文，再翻面確認</span></div>
          ...
          if(isPlaceholder&&!apiKey){
            return(<div className="srs-example-card" style={{textAlign:"center",color:S.t3}}>
              🌱 先記住這個單字的意思，就很棒了。<br/>
              <button className="srs-pill-btn" onClick={(e)=>{e.stopPropagation();onOpenSettings?.()}} style={{marginTop:8}}>請大人協助加入例句</button>
            </div>);
          }
      ...
      <div className="srs-dict-body">
        {(!apiKey?.trim()||dictError)&&<div data-testid="srs-local-dictionary" style={{padding:16,background:S.bg2,border:`1px solid ${S.bd}`,borderRadius:14,color:S.t2,lineHeight:1.8}}>
          <div style={{fontSize:12,color:c.cl,fontWeight:700}}>內建字典 · 直接學習</div>
          <h3 style={{fontSize:24,color:S.t1,margin:"8px 0"}}>{cur.m}</h3>
          <div>{partOfSpeechToZh(cur.p)} {cur.ph}</div>
          <button type="button" onClick={()=>speakDict(cur.w)} style={{...S.btn,background:c.cl,color:"#fff",padding:"8px 12px",fontSize:13,marginTop:10}}>播放單字</button>
          {cur.ex&&!isPlaceholderExample(cur.ex,cur.w)&&<div style={{paddingTop:14,marginTop:14,borderTop:`1px solid ${S.bd}`}}><b style={{color:S.t1}}>試著說一句</b><p style={{color:S.t1,margin:"8px 0"}}>{cur.ex} {speakTiny(cur.ex,"播放字典例句")}</p><p style={{margin:"4px 0",fontSize:14}}>{cur.ez}</p></div>}
    ...
    {flip&&<>
      ...
      <div data-testid="srs-rating-bar" className="srs-rating-bar">{[{k:"again",l:"Again",n:"1",cl:"#E24B4A",bg:"#FCEBEB",em:"😅"},{k:"hard",l:"Hard",n:"2",cl:"#BA7517",bg:"#FAEEDA",em:"🤔"},{k:"good",l:"Good",n:"3",cl:"#0F6E56",bg:"#E1F5EE",em:"😊"},{k:"easy",l:"Easy",n:"4",cl:"#185FA5",bg:"#E6F1FB",em:"🤩"}].map(b=>(<button className="srs-rate-btn" key={b.k} onClick={()=>rate(b.k)} style={{background:b.bg,color:b.cl}} onTouchStart={e=>e.currentTarget.style.transform="scale(0.93)"} onTouchEnd={e=>e.currentTarget.style.transform="scale(1)"} onMouseDown={e=>e.currentTarget.style.transform="scale(0.95)"} onMouseUp={e=>e.currentTarget.style.transform="scale(1)"}><span>{b.em}</span>{({again:"再看看",hard:"有點難",good:"記住了",easy:"很熟悉"})[b.k]}<small>{b.l} {b.n}</small></button>))}</div>
