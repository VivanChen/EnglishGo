...
export default function PetPlayground({pets,setPets,setCoins,c,onBack,incrTask,foods,getDef,levelUpPet,Hdr,speak,stopSpeech,playSound,compact=false}) {
  ...
  const SetupPages=compact?PetPages:Fragment;
  return <div className={`pet-world pet-playground ${compact?'pet-viewport pet-sanctuary pet-play-viewport':''}`}  data-testid="pet-playground">
    <Hdr t="🧺 夥伴遊樂園" onBack={run&&run.phase!=='done'?()=>pause(true):run?leave:onBack} cl={c.cl}/>
    <main className="pet-screen-body">
    {!run?<SetupPages>
      ...
      <section className="pet-game-reward-guide"><h3>完成後會得到什麼？</h3><p>每位夥伴每天每款遊戲首次完成：寵物 XP +18、親密 +8，體力 −4。有首次完成的夥伴時，這局另得金幣 +8。再玩可以提升星星紀錄，不重複發成長獎勵。</p></section>
    </SetupPages>:run.phase==='done'?<section className="pet-play-result">
      <SetupPages><div>
      ...
      <p>{run.points} 分 · 複習 {new Set(run.rounds.flatMap(r=>r.answers)).size} 個食物單字</p><p className="pet-note">完成得 1 星；失誤不超過 2 次再得 1 星；零失誤且沒用提示再得 1 星。</p></div><div className="pet-game-earned" role="status">{reward?.fresh?<><span>已存入獎勵</span><strong>金幣 +{reward.coins}</strong><p>{reward.fresh} 位夥伴各獲得：寵物 XP +18、親密 +8、陪伴印記 +1，體力 −4。</p></>:'今天的成長獎勵已領過，這次更新了最佳星星紀錄。'}</div>
      ...
      </SetupPages><div className="pet-button-row">{level<3&&<button className="pet-primary" onClick={()=>{leave();setLevel(level+1)}}>看看下一關 →</button>}<button className="pet-secondary" onClick={leave}>回遊樂園</button><button className="pet-secondary" onClick={onBack}>回家休息</button></div>
    </section>:<>
