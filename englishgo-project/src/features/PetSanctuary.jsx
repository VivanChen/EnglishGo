...
export default function PetSanctuary({onBack,onNavigate,onAccount,initialTab='home',c,pets,setPets,eggs,setEggs,coins,setCoins,inventory,setInventory,petAccount,setPetAccount,petTasks,incrTask,api}){
  ...
  return <div className="pet-world pet-home pet-sanctuary" data-testid="pet-sanctuary">
    ...
    :view==='shop'||view==='food'?<>
      ...
      <PetShowcase pets={pets} selected={selected} getDef={getDef} rarities={RARITY_INFO} onSelect={visit} onCare={startCare} motion={animLevel} result={result} care={care} foods={PET_FOODS} inventory={inventory} careReady={careReady} onReady={setCareReady} onCancel={()=>{setCare(null);stopSpeech?.()}} onFinish={finishCare} speak={speak} stopSpeech={stopSpeech} onShop={()=>{setCare(null);setView('shop')}}/>
      {result&&!care&&<section className="ps-notice ps-care-result" role="status" data-testid="pet-care-result"><div><b>照顧完成 · {CARE_CHOICES.find(a=>a.id===result.action)?.name}</b><p>{result.first?`金幣 +${result.rewards.coins} · XP +${result.rewards.exp}${result.rewards.bond?` · 親密 +${result.rewards.bond}`:''}`:'今天這項成長獎勵已領過，這次保留照顧效果。'}{result.combo?' · 三種陪伴獎勵已一起入帳':''}</p><small>今日培養 {Math.min(3,result.pet.careLog.actions.length)}/3</small></div></section>}
      {!care&&<PetGrowthPanel pet={selected} onChange={updated=>setPets(previous=>previous.map(p=>p.petId===updated.petId?updated:p))} onCare={()=>startCare(petNeed(selected).action)} careLabel={CARE_CHOICES.find(a=>a.id===petNeed(selected).action)?.name} onPlay={()=>setView('play')}/>}
      {!care&&<details className="ps-panel ps-details"><summary>夥伴故事、學習單字與成長能量</summary><p>{getDef(selected)?.story}</p><div className="pet-button-row">{(getDef(selected)?.words||[]).map(word=><button className="pet-secondary" key={word} onClick={()=>speak?.(word)}>{word} ♫</button>)}</div><p>成長能量 {selected.dupes||0} · 遇到重複夥伴時增加 XP、親密與遠征能力。原有技能與成長紀錄都保留。</p></details>}
    </>:<>
      {tab==='home'&&<>
