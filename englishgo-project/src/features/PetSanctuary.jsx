...
import PetShowcase from '../components/PetShowcase.jsx';
...
import PetPages from '../components/PetPages.jsx';
import '../components/pet-viewport.css';
...
export default function PetSanctuary({onBack,onNavigate,onAccount,initialTab='home',c,pets,setPets,eggs,setEggs,coins,setCoins,inventory,setInventory,petAccount,setPetAccount,petTasks,incrTask,api}){
  ...
  const [detail,setDetail]=useState('companion');
  useEffect(()=>{
    const old=document.body.style.overflow,background=[];document.body.style.overflow='hidden';
    let node=document.querySelector('.pet-viewport');
    while(node&&node!==document.body){for(const sibling of node.parentElement?.children||[]){if(sibling!==node&&!['SCRIPT','STYLE','LINK'].includes(sibling.tagName)){background.push([sibling,sibling.inert]);sibling.inert=true}}node=node.parentElement}
    return()=>{document.body.style.overflow=old;for(const [element,inert] of background)element.inert=inert};
  },[view]);
  useEffect(()=>{setCare(null);setCareReady(false)},[view,selectedId]);
  ...
  const visit=p=>{setSelectedId(p.petId);setResult(null);setDetail('companion')};
  const adopt=()=>{
  ...
  const startCare=(action,foodId)=>{
    if(!selected)return;setDetail('companion');
    const prompts=ACTION_PROMPTS[action]||['Let’s learn together.'];
    setCareReady(false);setCare({action,foodId,prompt:prompts[Math.floor(Math.random()*prompts.length)]});locks.current.delete('care');
  };
  const finishCare=()=>{
    if(!care||!selected||locks.current.has('care')||!careReady)return;
    ...
    setResult({...completed,action:care.action,before:{energy:selected.energy??80,hunger:selected.hunger??80,clean:selected.clean??80}});setCare(null);setView(null);playSound?.('good');
  };
  const buy=food=>{
  ...
  const activityCards=[{icon:'🧺',name:'夥伴遊樂園',tag:'輕鬆玩 · 2–4 分鐘',text:'野餐接力、記憶尋寶。三回合慢慢想，一位夥伴就能玩。',button:'去選遊戲',action:()=>setView('play')},{icon:'⛺',name:'森林遠征',tag:'英文闖關 · 約 5–10 分鐘',text:'最多三位夥伴組隊，答題、使用技能，在營地補給。',button:'準備探險',action:()=>onNavigate?.('petAdventure')},{icon:'🎲',name:'寵物大富翁',tag:'策略遊戲 · 較長的陪伴',text:'選路買地、機會命運、道具攻防。自由玩整局，英文小彩蛋由你選。',button:'前往寵物島',action:()=>onNavigate?.('petMonopoly')}];
  const activities=<PetPages className="ps-activity-grid" label="活動" columns={3} key={'ps-activity-grid'}>{activityCards.map((item,i)=><article key={item.name} className={`ps-activity ps-tone-${i}`}><span className="ps-activity-icon">{item.icon}</span><small>{item.tag}</small><h3>{item.name}</h3><p>{item.text}</p><button className="pet-secondary" disabled={!pets.length} onClick={item.action}>{pets.length?`${item.button} →`:'先孵出一位夥伴'}</button></article>)}</PetPages>;
  if(view==='play')return <PetPlayground compact {...{pets,setPets,setCoins,c,incrTask,levelUpPet,Hdr,speak,stopSpeech,playSound}} foods={PET_FOODS} getDef={getDef} onBack={()=>setView(null)}/>;
  return <div className="pet-world pet-home pet-sanctuary pet-viewport" data-testid="pet-sanctuary">
    <Hdr t="🐾 寵物小家園" cl={c.cl} onBack={care?()=>{setCare(null);stopSpeech?.()}:view?()=>{setView(null);setNotice('')}:selected?()=>setSelectedId(null):onBack} extra={<span className="ps-wallet" aria-label={`${coins} 學習金幣`}>🪙 {coins}</span>}/>
    ...
    <main className="pet-screen-body" key={view||(selected?'selected':tab)}>
    ...
    :view==='shop'||view==='food'?<>
      <SectionTitle eyebrow={view==='food'?'照顧 · 第 1 步':'小家補給站'} title={view==='food'?'今天想吃什麼？':'把喜歡的食物帶回家'}>{view==='food'?'先選一份食物，親手餵給夥伴吃。':'庫存與補充效果一起看。買好後回去餵食，英文發音可以另外點選。'}</SectionTitle>
      ...
      {!care&&<nav className="pet-detail-tabs" aria-label="陪伴分頁"><button aria-pressed={detail==='companion'} onClick={()=>setDetail('companion')}>立體陪伴</button><button aria-pressed={detail==='growth'} onClick={()=>setDetail('growth')}>成長與佈置</button><button aria-pressed={detail==='history'} onClick={()=>setDetail('history')}>照顧紀錄</button></nav>}
      {(detail==='companion'||care)&&<PetShowcase pets={pets} selected={selected} getDef={getDef} rarities={RARITY_INFO} onSelect={visit} onCare={startCare} motion={animLevel} result={result} care={care} foods={PET_FOODS} inventory={inventory} careReady={careReady} onReady={setCareReady} onCancel={()=>{setCare(null);stopSpeech?.()}} onFinish={finishCare} speak={speak} stopSpeech={stopSpeech} onShop={()=>{setCare(null);setView('shop')}}/>}
      {detail==='history'&&!care&&result&&<section className="ps-notice ps-care-result" role="status" data-testid="pet-care-result"><div><b>照顧完成 · {CARE_CHOICES.find(a=>a.id===result.action)?.name}</b><p>{result.first?`金幣 +${result.rewards.coins} · XP +${result.rewards.exp}${result.rewards.bond?` · 親密 +${result.rewards.bond}`:''}`:'今天這項成長獎勵已領過，這次保留照顧效果。'}{result.combo?' · 三種陪伴獎勵已一起入帳':''}</p><small>今日培養 {Math.min(3,result.pet.careLog.actions.length)}/3</small></div></section>}
      {detail==='growth'&&!care&&<PetGrowthPanel paged pet={selected} onChange={updated=>setPets(previous=>previous.map(p=>p.petId===updated.petId?updated:p))} onCare={()=>startCare(petNeed(selected).action)} careLabel={CARE_CHOICES.find(a=>a.id===petNeed(selected).action)?.name} onPlay={()=>setView('play')}/>}
      {detail==='history'&&!care&&<section className="ps-panel ps-details"><h3>夥伴故事、學習單字與成長能量</h3><p>{getDef(selected)?.story}</p><div className="pet-button-row">{(getDef(selected)?.words||[]).map(word=><button className="pet-secondary" key={word} onClick={()=>speak?.(word)}>{word} ♫</button>)}</div><p>成長能量 {selected.dupes||0} · 遇到重複夥伴時增加 XP、親密與遠征能力。原有技能與成長紀錄都保留。</p></section>}
    </>:<>
      {tab==='home'&&<>
        <section className="ps-hero"><div className="ps-hero-copy"><span className="pet-eyebrow">A LITTLE ENGLISH, A LITTLE CLOSER</span><h2>{pets.length?<>今天，也一起<br/>長大一點點。</>:<>你好，小夥伴。<br/>我們一起出發吧。</>}</h2><p>{pets.length?`${getDef(favorite)?.name||'小夥伴'}在等你。學一點英文，留一段回憶，陪伴不需要趕進度。`:'領養第一顆蛋，用三次英文學習迎接牠。照顧、玩耍和冒險，就從這裡開始。'}</p><button className="pet-primary" onClick={()=>pets.length?visit(favorite):goTab('eggs')}>{pets.length?'陪陪我的夥伴 →':'迎接第一位夥伴 →'}</button>{(ready.length>0||claimable.length>0)&&<button className="pet-link ps-priority" onClick={()=>goTab(ready.length?'eggs':'tasks')}>{ready.length?`${ready.length} 顆蛋可孵化 →`:`${claimable.length} 份任務獎勵可領取 →`}</button>}</div><PetHabitatScene pets={pets.length?[favorite]:[]} caption={pets.length?'一點陪伴，就是今天的小幸福':'一顆蛋，一段新友誼'}/></section>
      </>}
