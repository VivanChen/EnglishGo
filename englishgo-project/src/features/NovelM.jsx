...
export default function NovelM({lv,onBack,onXp,deps}){
  ...
  const readChapterZh=()=>{if(!chapter||!zhBlocks.length)return;stopNovelSpeech();novelSpeechRef.current=speakStory([chapter.zhTitle,...zhBlocks],{lang:"zh-TW",rate:1,apiTts:true,onSentence:i=>{const bi=i-1;if(bi>=0){setActiveBlock(bi);setPage(Math.floor(bi/NOVEL_PAGE_SIZE))}},onFinish:()=>{novelSpeechRef.current=null;setActiveBlock(null)},oncancel:()=>{novelSpeechRef.current=null;setActiveBlock(null)}})};
  ...
  const pageActionsStyle={display:"flex",gap:8,alignItems:"center",marginTop:14,...(isMobile?{position:"sticky",bottom:"0",zIndex:4,marginLeft:-2,marginRight:-2,paddingTop:10,paddingBottom:"calc(10px + env(safe-area-inset-bottom))",background:"linear-gradient(180deg,rgba(255,253,247,.72),#FFFDF7 42%)",boxShadow:"0 -10px 22px rgba(64,43,20,.07)",backdropFilter:"blur(8px)"}:{})};
  ...
  return(<div><Hdr t="📘 英文小說" onBack={backToList} cl={c.cl} extra={<button onClick={()=>setShowZh(z=>!z)} style={{background:"none",border:`1px solid ${S.bd}`,borderRadius:8,padding:"4px 8px",fontSize:12,color:c.cl,cursor:"pointer",fontFamily:"inherit"}}>{showZh?"隱藏中文":"顯示中文"}</button>}/>
    ...
    <div data-testid="novel-reader-panel" ref={novelPanelRef} style={{height:readingFocus?(isMobile?"clamp(520px, calc(100vh - 190px), 820px)":"clamp(560px, calc(100vh - 230px), 820px)"):(isMobile?"clamp(440px, calc(100vh - 260px), 720px)":"clamp(430px, calc(100vh - 330px), 720px)"),minHeight:0,overflowY:"auto",overflowX:"hidden",overscrollBehavior:"contain",scrollBehavior:"smooth",padding:isMobile?"0 4px calc(18px + env(safe-area-inset-bottom))":"0 4px 12px",border:`1px solid ${S.bd}`,borderRadius:14,background:"linear-gradient(180deg,rgba(255,255,255,.72),rgba(245,251,248,.66))"}}>
    <article style={{background:"#FFFDF7",border:`1px solid ${S.bd}`,borderRadius:8,padding:readingFocus?(isMobile?"16px 12px 18px":"18px 16px 20px"):(isMobile?"12px 10px 14px":"14px 13px 16px"),boxShadow:"0 8px 22px rgba(64,43,20,.08)",position:"relative",maxWidth:readingFocus?720:"none",margin:readingFocus?"0 auto":"0",minWidth:0}}>
      ...
      <div data-testid="novel-page-actions" style={pageActionsStyle}><button onClick={()=>turnPage(pageNow-1)} disabled={!canPrevPage} style={{...S.btn,background:S.bg2,color:S.t1,flex:1,padding:"10px",fontSize:13,opacity:canPrevPage?1:.4}}>上一頁</button><div style={{fontSize:12,color:S.t3,fontWeight:700,whiteSpace:"nowrap"}}>{pageNow+1}/{pages.length}</div><button onClick={()=>turnPage(pageNow+1)} disabled={!canNextPage} style={{...S.btn,background:c.cl,color:"#fff",flex:1,padding:"10px",fontSize:13,opacity:canNextPage?1:.4}}>下一頁</button></div>
    </article>
