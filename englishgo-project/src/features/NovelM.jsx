...
export default function NovelM({lv,onBack,onXp,deps}){
  ...
  useEffect(()=>{const target=Math.max(0,Number(pendingPageRef.current)||0);pendingPageRef.current=0;setPage(target);setActiveBlock(null);setActiveVocab(null);novelBlockRefs.current={};novelPanelRef.current?.scrollTo({top:0})},[ci,ni]);
  ...
  return(<div><Hdr t="📘 英文小說" onBack={backToList} cl={c.cl} extra={<button onClick={()=>setShowZh(z=>!z)} style={{background:"none",border:`1px solid ${S.bd}`,borderRadius:8,padding:"4px 8px",fontSize:12,color:c.cl,cursor:"pointer",fontFamily:"inherit"}}>{showZh?"隱藏中文":"顯示中文"}</button>}/>
    {!readingFocus&&<div style={{...S.card,padding:0,overflow:"hidden",marginBottom:10,border:`1px solid ${S.bd}`,borderTop:`4px solid ${c.cl}`,background:"linear-gradient(135deg,#FFFCF3,#F3FBF7)"}}>
      <div style={{display:"grid",gridTemplateColumns:"minmax(150px,220px) minmax(0,1fr)",gap:0,alignItems:"stretch"}}>
        <div style={{height:"clamp(170px,24vw,220px)",overflow:"hidden"}}><NovelIllustration chapter={chapter.no} imageBase={novelImageBase} title={novel.title}/></div>
        <div style={{padding:"16px",display:"grid",gap:12}}>
          <div style={{display:"flex",gap:12,alignItems:"flex-start",justifyContent:"space-between",flexWrap:"wrap"}}>
            <div style={{display:"flex",gap:12,alignItems:"flex-start",minWidth:0,flex:"1 1 260px"}}><div style={{width:42,height:42,borderRadius:"50%",background:c.cl,color:"#fff",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:900,flexShrink:0}}> {chapter.no}</div><div style={{minWidth:0}}><div style={{fontSize:12,color:c.cl,fontWeight:900}}>Chapter {chapter.no}</div><div style={{fontSize:22,fontWeight:900,color:S.t1,lineHeight:1.18}}>{chapter.title}</div><div style={{fontSize:13,color:S.t2,marginTop:3}}>{chapter.zhTitle} · {words} words</div></div></div>
            <div style={{display:"grid",gridTemplateColumns:"repeat(2,minmax(72px,1fr))",gap:7,flex:"0 1 190px"}}>
    ...
    <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap",marginBottom:10,padding:"10px 12px",border:`1px solid ${c.cl}33`,borderRadius:12,background:"linear-gradient(135deg,#FFFFFF,#F1FBF6)",boxShadow:"0 8px 20px rgba(15,110,86,.06)"}}>
      <div style={{fontSize:13,fontWeight:900,color:S.t1,marginRight:"auto"}}>閱讀設定</div>
      <button onClick={()=>updateReadingPrefs({fontSize:Math.max(14,readerFontSize-2)})} aria-label="A-" style={{...S.btn,background:S.bg2,color:S.t1,padding:"8px 10px",fontSize:12,minWidth:42}}>A-</button>
      <div style={{fontSize:12,fontWeight:900,color:c.cl,minWidth:42,textAlign:"center"}}>{readerFontSize}px</div>
      <button onClick={()=>updateReadingPrefs({fontSize:Math.min(22,readerFontSize+2)})} aria-label="A+" style={{...S.btn,background:c.cl,color:"#fff",padding:"8px 10px",fontSize:12,minWidth:42}}>A+</button>
      <button onClick={()=>updateReadingPrefs({lineHeight:readerLineHeight>=1.9?1.66:1.9})} style={{...S.btn,background:readerLineHeight>=1.9?c.bg:S.bg2,color:readerLineHeight>=1.9?c.cl:S.t1,padding:"8px 12px",fontSize:12}}>{readerLineHeight>=1.9?"一般行距":"寬行距"}</button>
      <button onClick={()=>updateReadingPrefs({focus:!readingFocus})} style={{...S.btn,background:readingFocus?c.cl:S.bg2,color:readingFocus?"#fff":S.t1,padding:"8px 12px",fontSize:12}}>{readingFocus?"離開專注":"專注模式"}</button>
    </div>
    <div style={{display:"grid",gridTemplateColumns:"auto minmax(0,1fr) auto",alignItems:"center",gap:8,marginBottom:10,fontSize:12,background:"#fff",border:`1px solid ${S.bd}`,borderRadius:12,padding:"9px 10px"}}><button onClick={()=>prev!=null&&goChapter(prev)} disabled={prev==null} style={{...S.btn,background:S.bg2,color:S.t1,padding:"8px 11px",opacity:prev==null?0.35:1,fontSize:12}}>← 上一章</button><div><div style={{display:"flex",justifyContent:"space-between",gap:8,marginBottom:5}}><span style={{fontWeight:900,color:S.t1}}>章節概覽</span><span style={{fontWeight:900,color:c.cl}}>{chapterPct}%</span></div><div style={{height:7,background:S.bg2,borderRadius:999,overflow:"hidden"}}><div style={{height:"100%",width:`${chapterPct}%`,background:`linear-gradient(90deg,${c.cl},${c.ac})`,borderRadius:999}}/></div></div><button onClick={()=>next!=null&&goChapter(next)} disabled={next==null} style={{...S.btn,background:S.bg2,color:S.t1,padding:"8px 11px",opacity:next==null?0.35:1,fontSize:12}}>下一章 →</button></div>
    <div ref={novelPanelRef} style={{height:readingFocus?"clamp(560px, calc(100vh - 230px), 820px)":"clamp(430px, calc(100vh - 330px), 720px)",minHeight:0,overflowY:"auto",overscrollBehavior:"contain",scrollBehavior:"smooth",padding:"0 4px 12px",border:`1px solid ${S.bd}`,borderRadius:14,background:"linear-gradient(180deg,rgba(255,255,255,.72),rgba(245,251,248,.66))"}}>
    <article style={{background:"#FFFDF7",border:`1px solid ${S.bd}`,borderRadius:8,padding:readingFocus?"18px 16px 20px":"14px 13px 16px",boxShadow:"0 8px 22px rgba(64,43,20,.08)",position:"relative",maxWidth:readingFocus?720:"none",margin:readingFocus?"0 auto":"0"}}>
      <div style={{position:"absolute",left:0,top:0,bottom:0,width:6,background:"linear-gradient(180deg,#E8D9B7,#F8F0D6)"}}/>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:8,marginBottom:12,paddingLeft:4,flexWrap:"wrap"}}><div><div style={{fontSize:12,fontWeight:900,color:c.cl}}>Page {pageNow+1}</div><div style={{fontSize:11,color:S.t3,fontWeight:800}}>段落 {pageStart+1}-{pageEnd} / {blockPairs.length}</div></div><div aria-label="本頁進度" style={{display:"flex",gap:4,alignItems:"center",justifyContent:"flex-end",overflowX:"auto",maxWidth:"100%",paddingBottom:2}}>{pages.map((_,i)=><button key={i} onClick={()=>turnPage(i)} aria-label={`前往第 ${i+1} 頁`} style={{width:i===pageNow?28:18,height:18,borderRadius:999,border:`1px solid ${i===pageNow?c.cl:S.bd}`,background:i===pageNow?c.cl:"#fff",display:"block",transition:"all .15s",cursor:"pointer",padding:0,flex:"0 0 auto"}} />)}</div></div>
