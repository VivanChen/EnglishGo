...
export default function NovelM({lv,onBack,onXp,deps}){
  ...
  const visiblePages=Array.from({length:visiblePageCount},(_,offset)=>({index:pageNow+offset,blocks:pages[pageNow+offset]||[]})).filter(item=>item.index<pages.length);
  ...
  useEffect(()=>{if(activeBlock!=null){const targetPage=findPageForBlock(pages,activeBlock);if(targetPage<pageNow||targetPage>=pageNow+visiblePageCount)setPage(spreadStartForPage(targetPage,visiblePageCount))}},[activeBlock,pageNow,pages,visiblePageCount]);
  ...
  const turnPage=(target,direction=0)=>{stopNovelSpeech();setActiveBlock(null);setTurnDirection(direction);setPage(spreadStartForPage(Math.max(0,Math.min(target,pages.length-1)),visiblePageCount));window.setTimeout?.(()=>setTurnDirection(0),420)};
  const goPreviousPage=()=>turnPage(previousSpreadStart(pageNow,visiblePageCount),-1);
  const goNextPage=()=>turnPage(nextSpreadStart(pageNow,pages.length,visiblePageCount),1);
  const handleReaderKeyDown=e=>{if(e.key==="ArrowLeft"&&canPrevPage){e.preventDefault();goPreviousPage()}if(e.key==="ArrowRight"&&canNextPage){e.preventDefault();goNextPage()}};
  ...
  const handlePointerUp=e=>{const start=swipeStartRef.current;swipeStartRef.current=null;if(!start)return;const dx=e.clientX-start.x;const dy=e.clientY-start.y;if(Math.abs(dx)<48||Math.abs(dx)<Math.abs(dy)*1.25)return;if(dx<0&&canNextPage)goNextPage();if(dx>0&&canPrevPage)goPreviousPage()};
  const renderNovelBlock=(b,measuring=false)=><section key={`${measuring?"measure":"read"}-${b.i}`} ref={el=>{if(measuring){if(el)measureBlockRefs.current[b.i]=el}else if(el)novelBlockRefs.current[b.i]=el}} onClick={measuring?undefined:()=>speakNovelText(b.en,"en-US",0.78,b.i)} style={{padding:"9px 10px",borderRadius:8,background:!measuring&&activeBlock===b.i?"#E6F7F0":"transparent",border:`1px solid ${!measuring&&activeBlock===b.i?c.cl:"transparent"}`,transition:"background .18s,border-color .18s",cursor:measuring?"default":"pointer"}}>
    <div style={{display:"flex",gap:8,alignItems:"flex-start"}}><p data-testid={measuring?undefined:"novel-reader-text"} style={{flex:1,margin:0,fontSize:readerFontSize,lineHeight:readerLineHeight,color:S.t1,fontWeight:/^“|^[A-Z][a-z]+[?!]?$/.test(b.en)?800:650,whiteSpace:"pre-line",overflowWrap:"anywhere"}}>{b.en}</p>{!measuring&&<button aria-label="朗讀英文" onClick={e=>{e.stopPropagation();e.currentTarget.blur();speakNovelText(b.en,"en-US",0.78,b.i)}} style={{width:34,height:34,border:`1px solid ${S.bd}`,background:S.bg1,borderRadius:10,padding:0,fontSize:13,cursor:"pointer",fontFamily:"inherit",color:c.cl,flexShrink:0}}>🔊</button>}</div>
    {showZh&&b.zh&&<div style={{marginTop:8,padding:"8px 10px",background:"#FFF7E6",border:"1px solid #F0D59A",borderRadius:8,fontSize:Math.max(13,readerFontSize-2),lineHeight:readerLineHeight,color:S.t2,whiteSpace:"pre-line",display:"flex",gap:8,alignItems:"flex-start",overflowWrap:"anywhere"}}><span style={{flex:1}}>{b.zh}</span>{!measuring&&<button onClick={e=>{e.stopPropagation();e.currentTarget.blur();speakNovelText(b.zh,"zh-TW",1,b.i)}} title="朗讀中文" style={{width:30,height:30,background:"#fff",border:"1px solid #F0D59A",borderRadius:9,fontSize:14,cursor:"pointer",flexShrink:0}}>🔈</button>}</div>}
  </section>;
  const renderPageSheet=item=><article key={item.index} data-testid="novel-book-page" style={{height:"100%",minWidth:0,overflow:"hidden",display:"flex",flexDirection:"column",background:"#FFFDF7",border:`1px solid ${S.bd}`,borderRadius:isMobile?8:item.index===pageNow?"8px 2px 2px 8px":"2px 8px 8px 2px",padding:isMobile?"14px 12px 12px":"16px 14px 12px",boxShadow:item.index===pageNow?"inset -14px 0 24px rgba(80,55,24,.05),0 10px 24px rgba(64,43,20,.1)":"inset 14px 0 24px rgba(80,55,24,.05),0 10px 24px rgba(64,43,20,.1)",transformOrigin:item.index===pageNow?"right center":"left center",animation:turnDirection?`novel-page-${turnDirection>0?"next":"previous"} .4s ease`:"none"}}>
  ...
  return(<div><Hdr t="📘 英文小說" onBack={backToList} cl={c.cl} extra={<button onClick={()=>setShowZh(z=>!z)} style={{background:"none",border:`1px solid ${S.bd}`,borderRadius:8,padding:"4px 8px",fontSize:12,color:c.cl,cursor:"pointer",fontFamily:"inherit"}}>{showZh?"隱藏中文":"顯示中文"}</button>}/>
    ...
    <style>{`@keyframes novel-page-next{0%{transform:perspective(1200px) rotateY(8deg);opacity:.72}100%{transform:perspective(1200px) rotateY(0);opacity:1}}@keyframes novel-page-previous{0%{transform:perspective(1200px) rotateY(-8deg);opacity:.72}100%{transform:perspective(1200px) rotateY(0);opacity:1}}@media (prefers-reduced-motion:reduce){[data-testid="novel-book-page"]{animation:none!important}}`}</style>
    <div data-testid="novel-reader-panel" ref={novelPanelRef} role="region" aria-label="小說閱讀器，可用左右方向鍵翻頁" tabIndex={0} onKeyDown={handleReaderKeyDown} onPointerDown={handlePointerDown} onPointerUp={handlePointerUp} style={{height:readingFocus?(isMobile?"clamp(520px, calc(100vh - 190px), 820px)":"clamp(560px, calc(100vh - 230px), 820px)"):(isMobile?"clamp(440px, calc(100vh - 260px), 720px)":"clamp(430px, calc(100vh - 330px), 720px)"),minHeight:0,overflowY:"hidden",overflowX:"hidden",overscrollBehavior:"contain",touchAction:"pan-y",padding:isMobile?"0 4px calc(18px + env(safe-area-inset-bottom))":"0 4px 12px",border:`1px solid ${S.bd}`,borderRadius:14,background:"linear-gradient(180deg,#E9E1D0,#F5F1E7)",display:"flex",flexDirection:"column",position:"relative",outline:"none"}}>
      <div ref={novelSpreadRef} data-testid="novel-book-spread" style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"repeat(2, minmax(0, 1fr))",gap:isMobile?0:2,flex:"1 1 auto",minHeight:0,padding:isMobile?"8px 4px 0":"10px 8px 0",position:"relative",perspective:1400}}>
        {!isMobile&&<div aria-hidden="true" style={{position:"absolute",zIndex:3,left:"50%",top:10,bottom:0,width:12,transform:"translateX(-50%)",background:"linear-gradient(90deg,rgba(84,58,29,.08),rgba(255,255,255,.7),rgba(84,58,29,.1))",boxShadow:"0 0 14px rgba(64,43,20,.12)",pointerEvents:"none"}}/>}
        {visiblePages.map(renderPageSheet)}
      </div>
      ...
      <div data-testid="novel-page-actions" style={pageActionsStyle}><button onClick={goPreviousPage} disabled={!canPrevPage} style={{...S.btn,background:S.bg2,color:S.t1,flex:1,padding:"10px",fontSize:13,opacity:canPrevPage?1:.4}}>上一頁</button><div style={{minWidth:92,textAlign:"center"}}><div style={{fontSize:12,color:S.t1,fontWeight:900,whiteSpace:"nowrap"}}>{isMobile?`Page ${pageNow+1}`:`Pages ${pageNow+1}-${Math.min(pageNow+2,pages.length)}`}</div><div style={{fontSize:10,color:S.t3}}>段落 {pageStart+1}-{pageEnd} / {blockPairs.length}</div></div><button onClick={goNextPage} disabled={!canNextPage} style={{...S.btn,background:c.cl,color:"#fff",flex:1,padding:"10px",fontSize:13,opacity:canNextPage?1:.4}}>下一頁</button></div>
    </div>
