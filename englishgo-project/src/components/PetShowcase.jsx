...
export default function PetShowcase({pets,selected,getDef,rarities,onSelect,onCare,motion,care,foods=[],inventory={},careReady,onReady,onCancel,onFinish,speak,stopSpeech,onShop,result}){
 const [panel,setPanel]=useState('status'),[touched,setTouched]=useState(false),[careInfo,setCareInfo]=useState(false);
 ...
 useEffect(()=>{
  const action=care?.action;setCareInfo(false);
 ...
 return <section ref={root} className={`pet-showcase ${careInfo?'shows-care-info':''} ${caring?'is-caring':''}`} aria-label="寵物星光展台" data-testid="pet-showcase">
  ...
  <nav className="showcase-roster" aria-label="切換寵物"><PetPages label="寵物選擇">{pets.map(p=><button key={p.petId} disabled={!!care} aria-pressed={p.petId===selected.petId} aria-label={`查看${getDef(p)?.name||p.petId}`} onClick={()=>{setTouched(false);onSelect(p)}}><PetCompanion petId={p.petId} size={62}/><span>{getDef(p)?.name||p.petId}</span><small>Lv.{p.level||1}</small></button>)}</PetPages></nav>
  ...
  <aside className="showcase-panel">
   {caring?<section className="showcase-care-panel" aria-label="照顧操作"><button className="care-info-toggle" aria-pressed={careInfo} onClick={()=>setCareInfo(!careInfo)}>{careInfo?'返回照顧':'獎勵與英文'}</button><h3>{care.action==='study'?'一起翻開小書':care.action==='sleep'?'留一段安靜的晚安':care.action==='play'?'來玩接球吧':care.action==='feed'?'一起吃點好吃的':'泡泡洗澡時間'}</h3>
    {care.action==='feed'&&!food?<><p>挑一份食物，親手餵給{def?.name}。</p><PetPages className="showcase-foods" label="餵食選擇">{foods.filter(f=>inventory[f.id]>0).map(f=><button key={f.id} aria-label={`選這份食物：${f.name}`} onClick={()=>onCare('feed',f.id)}><span>{f.emoji}</span><b>{f.name}<small>飽食 +{f.feed}</small></b><small>× {inventory[f.id]}</small></button>)}</PetPages>{!foods.some(f=>inventory[f.id]>0)&&<div className="showcase-empty-food"><p>食物盒空了，補充一份再回來。</p><button onClick={onShop}>去補食物 →</button></div>}</>:<><div className="care-explanation">{careInfo?<PetPages label="照顧說明">{care.action==='study'?<div className="showcase-care-preview"><span>今日讀書獎勵</span><b>XP +{completePetCare(selected,'study').rewards.exp}</b><small>{completePetCare(selected,'study').first?'完成並保存後獲得成長獎勵。':'今日獎勵已領取，仍可一起複習。'}</small></div>:sleepPreview?<div className="showcase-care-preview"><span>體力</span><b>{clamp(selected.energy)} → {clamp(sleepPreview.pet.energy)}</b><small>{sleepPreview.first?selected.energy>=100?'體力已滿，仍可留下一段晚安回憶。':'每日首次休息最多恢復 25 體力。':'今天已恢復過體力，這次安靜陪伴，不重複加成。'}</small></div>:playPreview?<div className="showcase-care-preview"><span>親密</span><b>{selected.bond||0} → {playPreview.pet.bond}</b><small>體力 {clamp(selected.energy)} → {clamp(playPreview.pet.energy)} · {playPreview.first?'今日首次玩耍獎勵':'今日獎勵已領取，這次不扣體力'}</small></div>:<div className="showcase-care-preview"><span>{care.action==='feed'?'飽食':'清潔'}</span><b>{clamp(selected[care.action==='feed'?'hunger':'clean'])} → {care.action==='feed'?Math.min(100,clamp(selected.hunger)+(food?.feed||0)):100}</b><small>完成後更新</small></div>}{care.action!=='study'&&<div className="showcase-care-english"><small>也可以說一句英文</small><blockquote>{care.prompt}</blockquote><button onClick={()=>speak?.(care.prompt)}>♫ 聽聽怎麼念</button></div>}<p className="showcase-care-note">{care.action==='study'?'讀完小書並夾上書籤後才可保存；途中取消不結算獎勵。':care.action==='sleep'?'完成晚安準備，再確認保存，才記錄恢復與成長獎勵。':care.action==='play'?'完成三回合後才結算；途中取消不扣體力。':care.action==='feed'?'完成時使用 1 份食物；取消不扣庫存。':'完成時才記錄清潔與成長獎勵。'}</p></PetPages>:<p>{careReady?'照顧完成！確認保存這次回憶。':'跟著畫面操作，完成後再保存。'}</p>}</div><button className="showcase-care-finish" data-testid="pet-action-complete" disabled={!careReady} onClick={onFinish}>{care.action==='sleep'?'保存晚安回憶 ✓':'完成照顧，收下回憶 ✓'}</button></>}
    <button className="showcase-care-cancel" onClick={onCancel}>稍後再做</button>
   </section>:<>

   {recent&&<div className="showcase-recent" role="status" data-testid="pet-care-result"><span>✓ 照顧完成</span><b>{CARE_CHOICES.find(a=>a.id===recent.action)?.name}完成</b><p>{recent.action==='sleep'?`體力 ${recent.before?.energy??0} → ${recent.pet.energy}`:recent.action==='feed'?`飽食 ${recent.before?.hunger??0} → ${recent.pet.hunger}`:recent.action==='clean'?`清潔 ${recent.before?.clean??0} → ${recent.pet.clean}`:recent.first?`親密 +${recent.rewards.bond} · XP +${recent.rewards.exp}`:'今天的成長獎勵已領取，謝謝你繼續陪伴。'}</p>{recent.first&&<small>金幣 +{recent.rewards.coins} · 陪伴印記 +1</small>}</div>}
